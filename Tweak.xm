...
%ctor{@autoreleasepool{NSString*b=NSBundle.mainBundle.bundleIdentifier?:@"";L(@"LOAD bundle=%@ process=%@",b,NSProcessInfo.processInfo.processName);if([b isEqualToString:@"com.apple.CarPlayApp"]&&!gLoop){gLoop=YES;dispatch_async(dispatch_get_main_queue(),^{Tick();});}}}
...
-(void)didMoveToWindow{ %orig; DumpHostEvent((UIView*)self,self.window?@"ATTACH":@"DETACH"); }
-(void)didMoveToSuperview{ %orig; DumpHostEvent((UIView*)self,self.superview?@"SUPER+":@"SUPER-"); }
